...
const items: Item[] = [
  ...
  { title: "For payers", href: "#payers" },
  { title: "Quickstart", href: "#quickstart" },
];

const RepoButton = ({ className, onClick }: { className?: string; onClick?: () => void }) => (
  <Button
    as={Link}
    href={REPO}
    target="_blank"
...
const NavLinks = ({ items }: { items: Item[] }) => (
...
const MobileNav = ({ items }: { items: Item[] }) => {
          ...
          >
            ...
            <div className="divide-divide border-divide mt-6 flex flex-col divide-y border-t">
              ...
              <div className="mt-4 p-4">
                <RepoButton className="w-full" onClick={() => setIsOpen(false)} />
              </div>
            </div>
          </motion.div>
        )}
...
const DesktopNav = ({ items }: { items: Item[] }) => (
...
const FloatingNav = ({ items }: { items: Item[] }) => {
  ...
  const y = useSpring(useTransform(scrollY, [100, 120], [-100, 10]), {
    stiffness: 300,
    damping: 30,
  });
  return (
